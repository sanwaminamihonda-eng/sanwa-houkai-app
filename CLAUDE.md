...
**デモ（`/demo/*`）と本番の機能内容は完全に一致させる。**

- デモ専用の機能を作らない
- 本番に機能追加したら、デモにも同じ機能を反映する
- UIコンポーネント、ビジネスロジックは共通化する
- 差異は認証の有無（デモは`@auth(level: PUBLIC)`）のみ

```
本番ページ: /schedule, /records, /clients, ...
デモページ: /demo/schedule, /demo/records, /demo/clients, ...
→ 機能・UI・動作は完全一致
```
